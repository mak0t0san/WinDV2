...
LRESULT CDVToolsDlg::OnDVTimeChange(WPARAM, LPARAM lParam)
{
	char buf[100] = "";
	if (lParam > 0) {
		// The timestamp travels as a 32-bit LPARAM, so widen it back into a
		// real time_t rather than aliasing it (time_t is 64-bit now).
		time_t recTime = (time_t)lParam;
		const struct tm *local = localtime(&recTime);
		if (local) strftime(buf, sizeof buf, "%d.%m.'%y %H:%M:%S", local);
	}
